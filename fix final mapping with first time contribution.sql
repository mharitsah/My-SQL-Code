...
WITH max_time AS (
  SELECT DATE_SUB(current_date(), INTERVAL 18 DAY) AS max_time_
),
filt_time AS(
SELECT 
    max_time_,
...
ga_1 AS(
    ...
    (CASE WHEN event_name = 'dax_testdrive_beforedealer_conversion' THEN 'conv' ELSE 'null' END) as conv_flag,
FROM 
...
ranked_utm AS (
  SELECT ga_session_id, phone_no, utm_source, log_time
...
phone_utm AS (
    ...
    LEFT JOIN 
        (SELECT DISTINCT phone_no, utm_source, utm_medium, utm_campaign, utm_content, ga_session_id, conv_flag, log_time FROM ga_utm) AS ga_utm_unique 
    ON ga_utm_grouped.ga_session_id = ga_utm_unique.ga_session_id AND ga_utm_grouped.utm_source = ga_utm_unique.utm_source AND ga_utm_grouped.phone_no = ga_utm_unique.phone_no AND ga_utm_grouped.log_time = ga_utm_unique.log_time
...
SELECT * FROM mapping_result AS m
WHERE NOT EXISTS (
    SELECT 1
