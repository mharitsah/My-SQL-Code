...
phone_utm AS (
    ...
    FROM 
        ...
        FROM 
            ga_utm 
        GROUP BY 
            phone_no) AS ga_utm_grouped
    LEFT JOIN 
        (SELECT DISTINCT phone_no, utm_source, utm_medium, utm_campaign, utm_content FROM ga_utm) AS ga_utm_unique 
    ON ga_utm_grouped.phone_no = ga_utm_unique.phone_no AND ga_utm_grouped.utm_source = ga_utm_unique.utm_source   
)
SELECT
    fullname,
    city,
